...
const { encodeParaSwapData, encodeParaSwapRepayAdapterParams } = require('../src/adapters');
const {
...
async function main() {
  ...
  const priceRoute = await paraswap.getRate(WETH[FORK_NETWORK_ID], DAI[FORK_NETWORK_ID], amount_to_swap.toString(), signer.address, 'BUY', { partner: 'aave', excludeContractMethods: ['simpleBuy'], });
  ...
  await (await aWETH.approve(
    adapter.address,
    ethers.BigNumber.from(priceWithSlippage).mul(10009).div(10000)
  )).wait();
  console.log('Allowance set successfully');

  // Perform the swap on the adapter
  console.log('Performing swap using flashLoan...');
  const params = encodeParaSwapRepayAdapterParams(
    DAI[FORK_NETWORK_ID],
