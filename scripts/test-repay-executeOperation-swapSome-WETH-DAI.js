...
async function main() {
  ...
  const amount_to_swap = ethers.utils.parseUnits('10000', 18);
  const paraswap = new ParaSwap(parseInt(FORK_NETWORK_ID), PARASWAP_API);
  const priceRoute = await paraswap.getRate(WETH[FORK_NETWORK_ID], DAI[FORK_NETWORK_ID], amount_to_swap.toString(), signer.address, 'BUY', { partner: 'aave', });
